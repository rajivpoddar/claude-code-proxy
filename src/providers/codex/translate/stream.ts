    ...
    async start(controller) {
      ...
      try {
        ...
        if (err instanceof UpstreamStreamError) {
          ...
          // an Anthropic `event: error` — no synthetic message_start /
          // content_block / message_delta / message_stop wrappers. Claude
          // Code's SSE parser surfaces `event: error` through
          // isApiErrorMessage and triggers normal retry/error UI.
          //
          // Tradeoff documented in feedback_raine_strict_error_shape_dropped_synthetic_recovery.md:
          // synthetic-recovery used to keep the usage accumulator consistent
          // (avoid `_.input_tokens` crash on /compact). Adopting strict
          // shape reintroduces that crash class — to be monitored on slot 1.
          const errorText =
            err.kind === "rate_limit"
          ...
          // message_start/content/message_delta/message_stop wrappers.
          // Tradeoff documented in feedback_raine_strict_error_shape_dropped_synthetic_recovery.md.
